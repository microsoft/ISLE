...
void ISLE::create_vocab_list(const std::string&        vocab_file,
...
std::string ISLE::log_dir_name(const doc_id_t     num_topics,
                               const std::string& output_path_base,
                               const bool&        sample_docs,
                               const FPTYPE&      sample_rate) {
  auto log_subdir = "log_t_" + std::to_string(num_topics) + "_eps1_" +
                    std::to_string(eps1_c) + "_eps2_" + std::to_string(eps2_c) +
                    "_eps3_" + std::to_string(eps3_c) + "_kMppReps_" +
                    std::to_string(KMEANS_INIT_REPS) + "_kMLowDReps_" +
                    std::to_string(MAX_KMEANS_LOWD_REPS) + "_kMReps_" +
                    std::to_string(MAX_KMEANS_REPS) + "_sample_" +
                    std::to_string(sample_docs);
  if (sample_docs)
...
std::string ISLE::concat_file_path(const std::string& dir,
...
#if defined(_MSC_VER)
uint64_t ISLE::open_win_mmapped_file_handle(const std::string filename,
  ...
  uint64_t fileSize =
      fileSizeLI.LowPart + (((uint64_t) fileSizeLI.HighPart) << 32);

  hMapFile = CreateFileMapping(
      hFile, NULL, PAGE_READONLY,  // default security & READ/WRITE mode
...
uint64_t ISLE::open_linux_mmapped_file_handle(const std::string filename,
                                              int& fd, void** buf) {
