...
namespace ISLE {
  ...
  template<class T>
  SparseMatrix<T>::SparseMatrix(const word_id_t d, const doc_id_t s,
                                const offset_t nnzs)
      : _vocab_size(d), _num_docs(s), _nnzs(nnzs), vals_CSC(NULL),
        rows_CSC(NULL), offsets_CSC(NULL), normalized_vals_CSC(NULL),
        avg_doc_sz((T) 0.0) {
  }
  ...
  SparseMatrix<T>::~SparseMatrix() {
    ...
    if (offsets_CSC)
      delete[] offsets_CSC;
    if (normalized_vals_CSC)
      delete[] normalized_vals_CSC;
  }
  ...
  template<class T>
  void SparseMatrix<T>::populate_CSC(
      const std::vector<DocWordEntry<count_t>>& entries) {
  ...
  template<class T>
  void SparseMatrix<T>::normalize_docs(bool delete_unnormalized,
                                       bool normalize_to_one) {
    normalized_vals_CSC = new T[get_nnzs()];
    uint64_t approx_empty_docs =
        0;  // Approx because there is race condition on counting in pfor

    pfor_static_131072(int64_t doc = 0; doc < num_docs(); ++doc) {
      auto doc_sum = std::accumulate(vals_CSC + offsets_CSC[doc],
                                     vals_CSC + offsets_CSC[doc + 1], (T) 0.0,
                                     std::plus<T>());
      if (doc_sum == (T) 0)
      ...
      for (offset_t pos = offsets_CSC[doc]; pos < offsets_CSC[doc + 1]; ++pos)
        if (std::is_same<T, count_t>::value) {
          ...
          normalized_vals_CSC[pos] = (count_t) std::ceil(
              (FPTYPE) avg_doc_sz * vals_CSC[pos] / doc_sum);
        } else if (std::is_same<T, FPTYPE>::value) {
          normalized_vals_CSC[pos] =
              normalize_to_one ? (FPTYPE) vals_CSC[pos] / (FPTYPE) doc_sum
                               : (FPTYPE) avg_doc_sz * ((FPTYPE) vals_CSC[pos] /
                                                        (FPTYPE) doc_sum);
        } else
          assert(false);
    }
  ...
  size_t SparseMatrix<T>::count_distint_top_five_words(int min_distinct) {
    ...
    std::sort(top_five.begin(), top_five.end(), [](const quintuple<T>& l,
                                                   const quintuple<T>& r) {
      return l.first < r.first || (l.first == r.first && l.second < r.second) ||
             (l.first == r.first && l.second == r.second &&
              l.third < r.third) ||
             (l.first == r.first && l.second == r.second &&
              l.third == r.third && l.fourth < r.fourth) ||
             (l.first == r.first && l.second == r.second &&
              l.third == r.third && l.fourth == r.fourth && l.fifth < r.fifth);
    });
    ...
    if (prev_iter - iter >= min_distinct)
      num_distinct++;
  ...
  template<class T>
  void SparseMatrix<T>::list_word_freqs_r(std::vector<T>* freqs,
                                          const doc_id_t  d_b,
                                          const doc_id_t  d_e,
                                          const word_id_t v_b,
                                          const word_id_t v_e) {
    if (d_e - d_b <= 16 && v_e - v_b <= 128) {
      for (doc_id_t doc = d_b; doc < d_e; ++doc) {
        if (offsets_CSC[doc] != offsets_CSC[doc + 1]) {
          auto row_b = std::lower_bound(rows_CSC + offsets_CSC[doc],
                                        rows_CSC + offsets_CSC[doc + 1], v_b);
          auto row_e = std::lower_bound(rows_CSC + offsets_CSC[doc],
                                        rows_CSC + offsets_CSC[doc + 1], v_e);
  ...
  void SparseMatrix<T>::list_word_freqs(std::vector<T>* freqs) {
    ...
    pfor(long long chunk = 0; chunk < num_chunks; chunk++) {
      ...
      auto chunk_e = (chunk + 1) * CHUNK_SIZE > num_docs()
                         ? num_docs()
                         : (chunk + 1) * CHUNK_SIZE;
  ...
  void SparseMatrix<T>::list_word_freqs_by_sorting(std::vector<A_TYPE>* freqs) {
    ...
    pfor_dynamic_1(int64_t chunk = 0; chunk < num_chunks; ++chunk) {
      ...
      auto off_e = offsets_CSC[(chunk + 1) * chunk_size > num_docs()
                                   ...
                                   : (chunk + 1) * chunk_size];
      std::sort(
          entries + off_b, entries + off_e, [](const auto& l, const auto& r) {
  ...
  template<class T>
  offset_t SparseMatrix<T>::compute_thresholds(std::vector<T>& zetas,
    ...
    const doc_id_t count_gr =
        (doc_id_t)(w0_c * (FPTYPE) num_docs() / (2.0 * (FPTYPE) num_topics));
    const doc_id_t count_eq = (doc_id_t) std::ceil(
        3.0 * eps1_c * w0_c * (FPTYPE) num_docs() / (FPTYPE) num_topics);

    for (word_id_t word = 0; word < vocab_size(); ++word) {
      assert(std::is_sorted(freqs[word].begin(), freqs[word].end(),
                            std::greater<>()));

      if (std::is_same<T, FPTYPE>::value) {
        for (auto iter = freqs[word].begin(); iter != freqs[word].end();
             ++iter) {
          assert(*iter <= avg_doc_sz);
          *iter = std::round(*iter);
        }
        auto trunc = std::lower_bound(freqs[word].begin(), freqs[word].end(),
                                      0.0, std::greater<>());
        if (trunc != freqs[word].end())
          assert(*trunc == 0.0);
        if (trunc != freqs[word].begin())
          assert(*(trunc - 1) > 0.0);
      ...
      if (freqs[word].size() > 0) {
        ...
        // Find zeta such that "#(freqs > zeta) > count_gr" & "#(freqs == zeta)
        // <=count_eq"
        if (std::is_same<T, count_t>::value) {
          auto zeta = freqs[word][count_gr - 1];
          while (1) {
            auto cur_pos = std::lower_bound(
                freqs[word].begin(), freqs[word].end(), zeta, std::greater<>());
            auto next_pos = std::upper_bound(
                freqs[word].begin(), freqs[word].end(), zeta, std::greater<>());
            ...
            if (next_pos - cur_pos < count_eq) {
              ...
              new_nnzs +=
                  next_pos - freqs[word].begin();  // Use freqs==zeta as well??
              zetas[word] = zeta;
              break;
            }
            // Check if the frequency counts end at zeta, or if zeta can go no
            // lower
          ...
          while (1) {
            ...
            assert((cur_pos != freqs[word].end()) && (cur_pos != next_pos));
            assert(zeta > 0);
            assert(zeta == *cur_pos && zeta == *(next_pos - 1));
            // Found exactly what we are looking for
            if (next_pos - cur_pos < count_eq) {
              // new_nnzs += cur_pos - freqs[word].begin();
              new_nnzs +=
  ...
  // Ouput: @thresholds: the @k-th highest count of each word in @doc_partition,
  // length = vocab_size()
  template<class T>
  void SparseMatrix<T>::rth_highest_element(
      const MKL_UINT r, const std::vector<doc_id_t>& doc_partition,
      T* thresholds) {
    {
      ...
      for (auto diter = doc_partition.begin(); diter != doc_partition.end();
           ++diter)
        for (auto witer = offsets_CSC[*diter];
             witer < offsets_CSC[(*diter) + 1]; ++witer)
      ...
      for (word_id_t word = 0; word < vocab_size(); ++word) {
        if (freqs[word].size() > r) {
          ...
          thresholds[word] = r >= doc_partition.size()
                                 ? freqs[word].size() == doc_partition.size()
                                       ? *std::min_element(freqs[word].begin(),
                                                           freqs[word].end())
                                       : (T) 0.0
                                 : (T) 0.0;
        }
  ...
  template<class T>
  void SparseMatrix<T>::find_catchwords(const doc_id_t          num_topics,
    ...
    for (doc_id_t topic = 0; topic < num_topics; ++topic) {
      for (word_id_t word = 0; word < vocab_size(); ++word) {
        ...
        for (doc_id_t other_topic = 0; other_topic < num_topics;
             ++other_topic) {
          if (topic != other_topic) {
            is_catchword =
                ((T) thresholds[word + (size_t) topic * (size_t) vocab_size()] >
                 rho_c * (T) thresholds[word +
                                        (size_t) other_topic *
                                            (size_t) vocab_size()]);
            if (!is_catchword)
              break;
  ...
  void SparseMatrix<T>::construct_topic_model(
    ...
    std::fill_n(Model.data(),
                (size_t) Model.vocab_size() * (size_t) Model.num_docs(),
                (FPTYPE) 0.0);
        ...
        if (avg_null_topics) {
          for (auto d_iter = closest_docs[topic].begin();
            ...
            for (offset_t pos = offsets_CSC[*d_iter];
              ...
              Model.elem_ref(rows_CSC[pos], topic) +=
                  (FPTYPE) normalized_vals_CSC[pos];
          for (word_id_t word = 0; word < vocab_size(); ++word)
    ...
    for (auto topic = 0; topic < num_topics; ++topic)
      if (catchwords[topic].size() > 0)
        for (auto iter = catchwords[topic].begin();
             iter < catchwords[topic].end(); ++iter)
    ...
    if (doc_blocks * doc_block_size != (size_t) num_docs())
      doc_blocks++;
    ...
    for (size_t block = 0; block < doc_blocks; ++block) {
      memset((void*) DocTopicSumArray, 0,
             (size_t) num_topics * num_docs_alloc * sizeof(FPTYPE));
      ...
      pfor(long long doc = doc_begin; doc < doc_end; ++doc) {
        offset_t c_pos = 0;
        for (offset_t pos = offsets_CSC[doc]; pos < offsets_CSC[doc + 1];
             ++pos) {
          while (c_pos < num_catchwords &&
                 all_catchwords[c_pos] < rows_CSC[pos])
            ++c_pos;
          if (c_pos == num_catchwords)
            break;

          if (all_catchwords[c_pos] == rows_CSC[pos])
            DocTopicSumArray[catchword_topic[c_pos] +
                             num_topics * (doc - block * doc_block_size)] +=
                normalized_vals_CSC[pos];
        }
      }
      for (doc_id_t doc = block * doc_block_size;
        ...
        for (doc_id_t topic = 0; topic < num_topics; ++topic) {
          if (DocTopicSumArray[topic +
                               num_topics * (doc - block * doc_block_size)])
            doc_topic_sum->push_back(std::make_tuple(
                doc, topic,
                DocTopicSumArray[topic +
                                 num_topics * (doc - block * doc_block_size)]));
        }
    ...
    for (doc_id_t doc = 0; doc < num_docs(); ++doc) {
      if (top_topic_pairs != NULL) {
        ...
        if (max_topic >= 0 && max2_topic >= 0) {
          ...
          top_topic_pairs->push_back(
              std::make_tuple(max_topic, max2_topic, doc));
        }
      }
    }
    ...
    /*FPTYPE *sorted_doc_topic_sums = new FPTYPE[(size_t)num_topics *
    (size_t)num_docs()];
    std::cout << "contruct topic model: sorted_doc_topic_sums alloc succeeded"
    << std::endl;
    ...
    for (auto topic = 0; topic < num_topics; ++topic)
        for (size_t block = 0; block < num_doc_blocks; ++block)
            for (doc_id_t doc = block*doc_block_size;
                doc < num_docs() && doc < (block + 1)*doc_block_size; ++doc)
                sorted_doc_topic_sums[(size_t)doc + (size_t)topic *
    (size_t)num_docs()]
                = DocTopicSums.elem(topic, doc);*/
    std::cout << "Size of doc_topic_sum array: " << doc_topic_sum->size()
              << std::endl;
    ...
    const size_t rank_threshold = (doc_id_t)(
        eps3_c * w0_c * (FPTYPE) num_docs() / ((FPTYPE) num_topics * 2.0));
    ...
    pfor_dynamic_1(long long topic_block = 0; topic_block < num_topic_blocks;
                   ++topic_block) {
      auto topic_begin = topic_block * topic_block_size;
      auto topic_end = (topic_block + 1) * topic_block_size;
      if (topic_end > num_topics)
        topic_end = num_topics;

      auto iter = std::lower_bound(
          doc_topic_sum->begin(), doc_topic_sum->end(), topic_begin,
          [](const auto& l, const auto& r) { return std::get<1>(l) < r; });
      for (auto topic = topic_begin; topic < topic_end; ++topic) {
        if (catchwords[topic].size() > 0) {
          /*std::sort(sorted_doc_topic_sums + (size_t)topic *
             (size_t)num_docs(),
              sorted_doc_topic_sums + (size_t)(topic + 1) * (size_t)num_docs(),
              std::greater<>());
              model_thresholds[topic]
              = sorted_doc_topic_sums[(size_t)topic * (size_t)num_docs() +
             (size_t)rank_threshold - 1];
              if (model_thresholds[topic] == 0.0)
              std::cout << "\n==== Warning: Topic " << topic << " threshold is
             0.\n";
          */
          auto end = std::upper_bound(
              iter, doc_topic_sum->end(), topic,
          ...
          if (end - iter < rank_threshold) {
            ...
            model_threshold =
                std::get<2>(*(iter + (size_t) rank_threshold - 1));
        ...
        if (catchwords[topic].size() > 0) {
            ...
            for (doc_id_t doc = 0; doc < num_docs(); ++doc) {
                if (DocTopicSums.elem(topic, doc) >= model_thresholds[topic]) {
                    num_cols_used++;
                    for (offset_t pos = offsets_CSC[doc]; pos < offsets_CSC[doc
    + 1]; ++pos)
                        Model.elem_ref(rows_CSC[pos], topic) +=
    (FPTYPE)normalized_vals_CSC[pos];
                }
    ...
    pfor_dynamic_1(int t = 0; t < num_topics; ++t) {
      auto topic_vector_sum =
          FPasum(Model.vocab_size(),
                 Model.data() + (size_t) Model.vocab_size() * (size_t) t, 1);
      FPscal(Model.vocab_size(), 1.0 / topic_vector_sum,
             Model.data() + (size_t) Model.vocab_size() * (size_t) t, 1);
    }
  ...
  void SparseMatrix<T>::topic_coherence(
      const doc_id_t num_topics, const word_id_t& M,
      const DenseMatrix<FPTYPE>& model,
      std::vector<std::pair<word_id_t, FPTYPE>>* top_words,
      std::vector<FPTYPE>& coherences, const FPTYPE coherence_eps) {
    assert(model.vocab_size() == vocab_size());
    for (auto topic = 0; topic < num_topics; ++topic)
    ...
    for (auto topic = 0; topic < num_topics; ++topic) {
      if (top_words[topic].size() > 1)
        pfor_dynamic_8192(long long word = 0; word < M;
                          ++word) for (word_id_t word2 = 0; word2 < word;
                                       ++word2) {
          assert(doc_freqs[topic][word2] > 0);
          coherences[topic] +=
              (FPTYPE) std::log(joint_doc_freqs[topic][word][word2] +
                                coherence_eps) -
  ...
  void SparseMatrix<T>::compute_joint_doc_frequency(
    ...
    if (!use_parallel_version) {  // Serial version
      for (doc_id_t doc = 0; doc < num_docs(); ++doc)
        for (auto topic = 0; topic < num_topics; ++topic)
      ...
      pfor_dynamic_1(int64_t chunk = 0; chunk < num_chunks; ++chunk) for (
          int64_t doc = chunk * chunk_size;
          doc < num_docs() && doc < (chunk + 1) * chunk_size;
          ++doc) for (auto topic = 0; topic < num_topics;
                      ++topic) for (word_id_t w1 = 0;
                                    w1 < top_words[topic].size();
                                    ++w1) for (word_id_t w2 = 0; w2 < w1;
                                               ++w2) if (normalized(top_words
                                                                        [topic]
                                                                        [w1].first,
                                                                    doc) > 0 &&
                                                         normalized(
  ...
  void SparseMatrix<T>::compute_doc_frequency(
    ...
    if (!use_parallel_version) {  // Use Serial version
      ...
      for (size_t chunk = 0; chunk < num_chunks; ++chunk) {
        ...
        for (auto topic = 0; topic < num_topics; ++topic) {
          assert(top_words[topic].size() > 0);
          doc_frequencies_chunks[chunk][topic].resize(top_words[topic].size(),
                                                      0);
        }
      }
      ...
      pfor_dynamic_1(int64_t chunk = 0; chunk < num_chunks; ++chunk) for (
          ...
          doc < num_docs() && doc < (chunk + 1) * chunk_size;
          ++doc) for (auto topic = 0; topic < num_topics;
                      ++topic) for (word_id_t word = 0;
                                    word < top_words[topic].size();
                                    ++word) if (normalized(top_words[topic]
                                                                    [word]
                                                                        .first,
                                                           doc) > (T) 0.0)
  ...
  template<class T>
  void SparseMatrix<T>::compute_log_combinatorial(
      std::vector<FPTYPE>& docs_log_fact) {
  ...
  FloatingPointSparseMatrix<FPTYPE>::FloatingPointSparseMatrix(
      const word_id_t d, const doc_id_t s)
      : SparseMatrix<FPTYPE>(d, s), U_colmajor(NULL), U_rows(0), U_cols(0),
        SigmaVT(NULL) {
  }
  ...
  template<class FPTYPE>
  FloatingPointSparseMatrix<FPTYPE>::FloatingPointSparseMatrix(
      const SparseMatrix<FPTYPE>& from, const bool copy_normalized)
    ...
    if (copy_normalized) {
      ...
      memcpy(vals_CSC, from.normalized_vals_CSC,
             (size_t) _nnzs * sizeof(FPTYPE));
      memcpy(normalized_vals_CSC, from.normalized_vals_CSC,
             (size_t) _nnzs * sizeof(FPTYPE));
    } else {
    ...
    memcpy(offsets_CSC, from.offsets_CSC,
           ((size_t) num_docs() + 1) * sizeof(offset_t));
  }
  ...
  FPTYPE FloatingPointSparseMatrix<FPTYPE>::normalized_frobenius() const {
    assert(normalized_vals_CSC != NULL);
    return FPdot(get_nnzs(), normalized_vals_CSC, 1, normalized_vals_CSC, 1);
  }

  template<class FPTYPE>
  FloatingPointSparseMatrix<FPTYPE>::WordDocPair::WordDocPair(
      const word_id_t& word_, const doc_id_t& doc_)
      : word(word_), doc(doc_) {
  }

  template<class FPTYPE>
  FloatingPointSparseMatrix<FPTYPE>::WordDocPair::WordDocPair(
      const WordDocPair& from) {
    word = from.word;
    doc = from.doc;
  }
  ...
  void FloatingPointSparseMatrix<FPTYPE>::get_word_major_list(
      std::vector<WordDocPair>& entries, std::vector<offset_t>& word_offsets) {
    ...
    for (word_id_t word = 0; word < vocab_size(); ++word)
      word_offsets.push_back(
          std::lower_bound(
              entries.begin(), entries.end(), WordDocPair(word, 0),
  ...
  template<class FPTYPE>
  void FloatingPointSparseMatrix<FPTYPE>::initialize_for_eigensolver(
      const doc_id_t num_topics) {
    U_rows = vocab_size();
    U_cols = num_topics;
  ...
  void FloatingPointSparseMatrix<FPTYPE>::compute_Spectra(
      const doc_id_t num_topics, std::vector<FPTYPE>& evalues) {
    MKL_SpSpTrProd<FPTYPE> op(vals_CSC, rows_CSC, offsets_CSC, vocab_size(),
                              num_docs(), get_nnzs());
    Spectra::SymEigsSolver<FPTYPE, Spectra::LARGEST_ALGE,
                           MKL_SpSpTrProd<FPTYPE>>
  ...
  void FloatingPointSparseMatrix<FPTYPE>::compute_block_ks(
    ...
    BlockKs<MKL_SpSpTrProd<FPTYPE>> eigensolver(
        &op, num_topics, 2 * num_topics + BLOCK_KS_BLOCK_SIZE,
  ...
  void FloatingPointSparseMatrix<FPTYPE>::compute_sigmaVT(
    ...
    auto tr_diff = std::abs(FPdot((size_t) vocab_size() * (size_t) num_topics,
                   ...
                   FPdot((size_t) vocab_size() * (size_t) num_topics, U_rowm, 1,
                         U_rowm, 1);
    if (tr_diff > 0.01)
    ...
    FPcsrmm(&transa, &m, &n, &k, &alpha, matdescra, vals_CSC,
            (const MKL_INT*) rows_CSC, (const MKL_INT*) offsets_CSC,
            (const MKL_INT*) (offsets_CSC + 1), U_rowm, &n, &beta, SigmaVT, &n);
    delete[] U_rowm;
  }
  ...
  // Input: @nnzs: Number of nnz elements that would remain after thresholding,
  // pre-calculated
  ...
  void FloatingPointSparseMatrix<FPTYPE>::threshold_and_copy(
      const SparseMatrix<fromT>& from, const std::vector<fromT>& zetas,
      const offset_t nnzs, std::vector<doc_id_t>& original_cols) {
    assert(vocab_size() == from.vocab_size() && num_docs() == from.num_docs());
    assert(original_cols.size() == 0);
    assert(zetas.size() == vocab_size());
    ...
    for (doc_id_t doc = 0; doc < num_docs(); ++doc) {
      for (offset_t pos = from.offsets_CSC[doc];
           pos < from.offsets_CSC[doc + 1]; ++pos) {
        fromT val;
    ...
    if (offsets_CSC[nz_docs] != get_nnzs() - extra) {
      ...
      std::cout
          << " ************ WARNING: last offset != allocation ************* \n"
          << " ************ Resetting nnzs ********************************* "
             "\n\n";
  ...
  void FloatingPointSparseMatrix<FPTYPE>::sampled_threshold_and_copy(
    ...
    pfor_dynamic_131072(int64_t doc = 0; doc < num_docs(); ++doc) {
      dice[doc] = doc_weights[doc] == 0.0
                      ? 0.0
                      : std::pow(rand_fraction(), 1 / doc_weights[doc]);
      doc_weights[doc] = dice[doc];
    }
    std::nth_element(dice,
                     dice + (size_t)(sample_rate * (FPTYPE) from.num_docs()),
                     dice + from.num_docs(), std::greater<>());
  ...
  void FloatingPointSparseMatrix<FPTYPE>::left_multiply_by_U_Spectra(
    ...
    FPgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, (MKL_INT) vocab_size(),
           (MKL_INT) ncols, (MKL_INT) U_cols, (FPTYPE) 1.0, U_colmajor,
           (MKL_INT) vocab_size(), in, (MKL_INT) ld_in, (FPTYPE) 0.0, out,
           (MKL_INT) U_rows);
  }

  template<class FPTYPE>
  void FloatingPointSparseMatrix<FPTYPE>::copy_col_to(
      FPTYPE* const dst, const doc_id_t doc) const {
  ...
  inline FPTYPE FloatingPointSparseMatrix<FPTYPE>::distsq_doc_to_pt(
      const doc_id_t doc, const FPTYPE* const pt, const FPTYPE pt_l2sq) const {
    FPTYPE ret =
        (pt_l2sq == -1.0) ? FPdot(vocab_size(), pt, 1, pt, 1) : pt_l2sq;
    ret +=
        FPdot(offsets_CSC[doc + 1] - offsets_CSC[doc],
              vals_CSC + offsets_CSC[doc], 1, vals_CSC + offsets_CSC[doc], 1);
    for (auto witer = offsets_CSC[doc]; witer < offsets_CSC[doc + 1]; ++witer)
  ...
  void FloatingPointSparseMatrix<FPTYPE>::distsq_alldocs_to_centers(
      ...
      const FPTYPE* const centers_l2sq, const FPTYPE* const docs_l2sq,
      FPTYPE* dist_matrix) {
    assert(sizeof(MKL_INT) == sizeof(offset_t));
    ...
    FPcsrmm(&transa, &m, &n, &k, &alpha, matdescra, vals_CSC,
            (const MKL_INT*) rows_CSC, (const MKL_INT*) offsets_CSC,
            (const MKL_INT*) (offsets_CSC + 1), centers_tr, &n, &beta,
            dist_matrix, &n);

    FPgemm(CblasRowMajor, CblasTrans, CblasNoTrans, num_docs(), num_centers, 1,
      ...
      FPTYPE* const dist_matrix)  // Initialized to num_centers*num_docs() size
  {
    ...
    pfor_static_131072(int64_t d = 0; d < num_docs(); ++d) center_index[d] =
  ...
  FPTYPE FloatingPointSparseMatrix<FPTYPE>::lloyds_iter(
    ...
    pfor_dynamic_1(int c = 0; c < num_centers; ++c) {
      ...
      for (auto diter = closest_docs[c].begin(); diter != closest_docs[c].end();
           ++diter)
        for (auto witer = offsets_CSC[*diter]; witer < offsets_CSC[1 + *diter];
             ++witer)
    ...
    if (compute_residual) {
      ...
      pfor(int chunk = 0; chunk < nchunks;
          ...
          residuals[chunk * BUF_PAD] += distsq_doc_to_pt(
              d, centers + (size_t) closest_center[d] * (size_t) vocab_size());
  ...
  template<class FPTYPE>
  void FloatingPointSparseMatrix<FPTYPE>::compute_docs_l2sq(
      FPTYPE* const docs_l2sq) {
    assert(docs_l2sq != NULL);
    FPscal(num_docs(), 0.0, docs_l2sq, 1);
    pfor_static_131072(int64_t d = 0; d < num_docs();
                       ++d) for (auto witer = offsets_CSC[d];
                                 witer < offsets_CSC[d + 1]; ++witer)
  ...
  FPTYPE FloatingPointSparseMatrix<FPTYPE>::run_lloyds(
      const doc_id_t num_centers, FPTYPE* centers,
      std::vector<doc_id_t>*
                closest_docs,  // Pass NULL if you dont want closest_docs
      const int max_reps) {
    FPTYPE residual;
  ...
  FPTYPE FloatingPointSparseMatrix<FPTYPE>::run_elkans(
    ...
    */
    ...
    FPTYPE* const lb_distsq_matrix =
        new FPTYPE[(size_t) num_centers * (size_t) num_docs()];
    FPTYPE* const centers_distsq =
        new FPTYPE[(size_t) num_centers * (size_t) num_centers];
    doc_id_t* const closest_center = new doc_id_t[num_docs()];
    FPTYPE* const   centers_l2sq = new FPTYPE[num_centers];
    ...
    //	ub_distsq[doc] = distsq_doc_to_pt(doc, centers + vocab_size() *
    // closest_center[doc]);
    ...
    pfor_dynamic_16(auto c = 0; c < num_centers; ++c) {
      centers_l2sq[c] =
          FPdot(vocab_size(), centers + (size_t) c * (size_t) vocab_size(), 1,
                centers + (size_t) c * (size_t) vocab_size(), 1);
      for (auto diter = nearest_docs[c].begin(); diter != nearest_docs[c].end();
           ++diter)
        ub_distsq[*diter] = distsq_doc_to_pt(
            *diter, centers + (size_t) vocab_size() * (size_t) c,
            centers_l2sq[c]);
    }
    delete[] nearest_docs;
    ...
    for (int rep = 0; rep < MAX_KMEANS_REPS; ++rep) {
      ...
      pfor_static_256(auto i = 0; i < num_centers; ++i) {
        centers_l2sq[i] =
            FPdot(vocab_size(), centers + (size_t) i * (size_t) vocab_size(), 1,
                  centers + (size_t) i * (size_t) vocab_size(), 1);
        ones_vec[i] = 1.0;
      }
      {
        FPgemm(CblasColMajor, CblasTrans, CblasNoTrans, num_centers,
               num_centers, vocab_size(), (FPTYPE) -2.0, centers, vocab_size(),
               centers, vocab_size(), (FPTYPE) 0.0, centers_distsq,
               num_centers);
        FPgemm(CblasColMajor, CblasNoTrans, CblasTrans, num_centers,
               num_centers, 1, (FPTYPE) 1.0, ones_vec, num_centers,
               centers_l2sq, num_centers, (FPTYPE) 1.0, centers_distsq,
               num_centers);
        FPgemm(CblasColMajor, CblasNoTrans, CblasTrans, num_centers,
               num_centers, 1, (FPTYPE) 1.0, centers_l2sq, num_centers,
               ones_vec, num_centers, (FPTYPE) 1.0, centers_distsq,
               num_centers);
      }
      std::vector<FPTYPE> distsq_nearest_other_center(num_centers);
      pfor_dynamic_1024(auto c = 0; c < num_centers; ++c) {
        auto imin1 =
            FPimin(c, centers_distsq + (size_t) c * (size_t) num_centers, 1);
        auto imin2 = FPimin(
            num_centers - 1 - c,
            centers_distsq + (size_t) c * (size_t) num_centers + c + 1, 1);
        auto min1 = centers_distsq[(size_t) c * (size_t) num_centers + imin1];
        auto min2 =
            centers_distsq[(size_t) c * (size_t) num_centers + c + 1 + imin2];
      ...
      // (b) If d(x,c(x)) > l(x,c) or d(x,c(x)) > 1/2 * d(c(x),c), compute
      // d(x,c).
      //              Now, if d(x,c) < d(x,c(x)), then assign c(x)=c & update
      //              u(x) = d(x,c(x)).
      auto nearest_docs = new std::vector<doc_id_t>[num_centers];
      for (doc_id_t doc = 0; doc < num_docs(); ++doc)
        nearest_docs[closest_center[doc]].push_back(doc);
      pfor_dynamic_16(auto c = 0; c < num_centers;
                      ++c) for (auto diter = nearest_docs[c].begin();
                                diter != nearest_docs[c].end();
                                ++diter) ub_distsq[*diter] =
          distsq_doc_to_pt(*diter, centers + (size_t) vocab_size() * (size_t) c,
                           centers_l2sq[c]);
      ...
      pfor_dynamic_1(auto batch = 0; batch < num_doc_batches;
                     ++batch) for (doc_id_t doc = batch * doc_batch_size;
                                   doc < (batch + 1) * doc_batch_size &&
                                   doc < num_docs();
                                   ++doc) {
        ...
        if (distsq_to_current_center < 0.25 * distsq_nearest_other_center[c])
          continue;
        auto imin1 = FPimin(
            c, lb_distsq_matrix + (size_t) doc * (size_t) num_centers, 1);
        auto imin2 = FPimin(
            num_centers - 1 - c,
            lb_distsq_matrix + (size_t) doc * (size_t) num_centers + c + 1, 1);
        auto min1 =
            lb_distsq_matrix[(size_t) doc * (size_t) num_centers + imin1];
        auto min2 = lb_distsq_matrix[(size_t) doc * (size_t) num_centers + c +
                                     1 + imin2];
      ...
      size_t moved_docs = 0;
      for (doc_id_t b = 0; b < num_doc_batches; ++b)
        moved_docs += moving_docs[b].size();
      ...
      pfor_dynamic_1(auto batch = 0; batch < num_doc_batches; ++batch) {
        size_t buffer_sz = 0;
        for (auto diter = moving_docs[batch].begin();
             diter != moving_docs[batch].end(); ++diter)
        ...
        auto buffer_dist_matrix =
            new FPTYPE[moving_docs[batch].size() * (size_t) num_centers];
        ...
        for (size_t i = 0; i != moving_docs[batch].size(); ++i) {
          ...
          memcpy(buffer_vals_CSC + buffer_offsets_CSC[i],
                 vals_CSC + offset_CSC(doc),
                 sizeof(FPTYPE) * (size_t) num_nnzs);
          memcpy(buffer_rows_CSC + buffer_offsets_CSC[i],
                 rows_CSC + offset_CSC(doc),
                 sizeof(word_id_t) * (size_t) num_nnzs);
        ...
        FPcsrmm(&transa, &m, &n, &k, &alpha, matdescra, buffer_vals_CSC,
                (const MKL_INT*) buffer_rows_CSC,
                (const MKL_INT*) buffer_offsets_CSC,
                (const MKL_INT*) (buffer_offsets_CSC + 1), centers_tr, &n,
                &beta, buffer_dist_matrix, &n);
        FPgemm(CblasRowMajor, CblasTrans, CblasNoTrans, m, n, 1, (FPTYPE) 1.0,
               ones_vec, m, centers_l2sq, n, (FPTYPE) 1.0, buffer_dist_matrix,
               n);
        FPgemm(CblasRowMajor, CblasTrans, CblasNoTrans, m, n, 1, (FPTYPE) 1.0,
               docs_l2sq, m, ones_vec, n, (FPTYPE) 1.0, buffer_dist_matrix, n);
        delete[] ones_vec;
        delete[] centers_tr;
        ...
        for (size_t i = 0; i < moving_docs[batch].size(); ++i) {
          ...
          memcpy(lb_distsq_matrix + (size_t) doc * (size_t) num_centers,
                 buffer_dist_matrix + (size_t) i * (size_t) num_centers,
                 num_centers * sizeof(FPTYPE));
          closest_center[doc] =
              FPimin(num_centers,
                     lb_distsq_matrix + (size_t) doc * (size_t) num_centers, 1);
          ub_distsq[doc] =
              lb_distsq_matrix[(size_t) doc * (size_t) num_centers +
                               closest_center[doc]];
        }
        delete[] buffer_vals_CSC, buffer_rows_CSC, buffer_offsets_CSC,
            buffer_dist_matrix;
      }
      ...
      auto new_centers =
          new FPTYPE[(size_t) vocab_size() * (size_t) num_centers];
      FPscal((size_t) vocab_size() * (size_t) num_centers, 0.0, new_centers, 1);
      pfor_dynamic_16(auto c = 0; c < num_centers; ++c) {
        auto div = (FPTYPE) closest_docs[c].size();
        for (auto diter = closest_docs[c].begin();
             diter != closest_docs[c].end(); ++diter)
          for (auto witer = offset_CSC(*diter); witer < offset_CSC(1 + *diter);
               ++witer)
            *(new_centers + (size_t) c * (size_t) vocab_size() +
              row_CSC(witer)) += val_CSC(witer) / div;
      }
      ...
      FPaxpy((size_t) vocab_size() * (size_t) num_centers, -1.0, new_centers, 1,
             centers, 1);
      pfor_dynamic_16(auto c = 0; c < num_centers; ++c) distsq_center_mv[c] =
          FPdot(vocab_size(), centers + (size_t) c * (size_t) vocab_size(), 1,
                centers + (size_t) c * (size_t) vocab_size(), 1);

      pfor_dynamic_1(auto batch = 0; batch < num_doc_batches;
                     ++batch) for (auto diter = moving_docs[batch].begin();
                                   diter != moving_docs[batch].end(); ++diter) {
        for (auto c = 0; c < num_centers; ++c) {
          if (lb_distsq_matrix[(*diter) * (size_t) num_centers + (size_t) c] <
              distsq_center_mv[c])
            ...
            lb_distsq_matrix[(*diter) * (size_t) num_centers + (size_t) c] =
                std::pow(
                    std::sqrt(lb_distsq_matrix[(*diter) * (size_t) num_centers +
                                               (size_t) c]) -
                        std::sqrt(distsq_center_mv[c]),
                    2);
        }
        ub_distsq[*diter] =
            std::pow(std::sqrt(ub_distsq[*diter]) +
                         std::sqrt(distsq_center_mv[closest_center[*diter]]),
                     2);
      }

      // 7. Replace c by m(c).
      // FPblascopy((size_t)num_centers* (size_t)vocab_size(), new_centers, 1,
      // centers, 1);
      memcpy(centers, new_centers,
...
template void ISLE::FloatingPointSparseMatrix<float>::threshold_and_copy(
    const SparseMatrix<float>& from, const std::vector<float>& zetas,
    const offset_t nnzs, std::vector<doc_id_t>& original_cols);

template void
ISLE::FloatingPointSparseMatrix<float>::sampled_threshold_and_copy(
    const SparseMatrix<float>& from, const std::vector<float>& zetas,
    const offset_t nnzs, std::vector<doc_id_t>& original_cols,
    const FPTYPE sample_rate);
