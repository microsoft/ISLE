...
namespace ISLE {
  class LogUtils {
   public:
    ...
    void print_stringstream(const std::ostringstream& stream,
                            const bool print_to_terminal = true) {
    ...
    void print_catch_words(const doc_id_t topic, const catchT* catch_threshold,
    ...
    void print_cluster_details(const doc_id_t                      num_topics,
      ...
      for (auto i = 0; i < num_topics; ++i) {
        ...
        ostr << std::setw(12) << std::left << "Cluster" << t << std::setw(12)
             << std::left << "  size:" << cluster_sizes[i].first
      ...
      ostr << "\n#Topics with no catchwords: " << catchless << "(" << num_topics
           << ")" << std::endl;
    ...
    void print_eigen_data(std::vector<FPTYPE>& evalues, doc_id_t num_topics,
                          bool print_to_terminal = true) {
      std::ostringstream ostr;
      ...
      for (auto slab = 0; slab < num_topics / 100; ++slab)
        ostr << "Sum of Top-" << (slab + 1) * 100 << " eig vals: "
             << std::accumulate(eig_sum_slabs.begin(),
                                eig_sum_slabs.begin() + 1 + slab, (FPTYPE) 0.0)
             << "\n";
