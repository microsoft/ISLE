...
namespace ISLE {
...
#if defined(_MSC_VER)
  uint64_t open_win_mmapped_file_handle(const std::string filename,
                                        HANDLE& hFile, HANDLE& hMapFile,
                                        char** pBuf);
...
#if defined(LINUX)
  uint64_t open_linux_mmapped_file_handle(const std::string filename, int& fd,
                                          void** buf);

  void close_linux_mmapped_file_handle(int fd, void* buf, off_t fileSize);
#endif
  ...
  struct DocWordEntry {
    ...
    DocWordEntry() : word(0), doc(0), count((COUNT_T) 0) {
    }

    DocWordEntry(const word_id_t& word_, const doc_id_t& doc_,
                 const COUNT_T& count_)
        : word(word_), doc(doc_), count(count_) {
    }
  ...
  struct DocWordEntriesReader {
    ...
    void read_from_file(const std::string& filename,
...
#if FILE_IO_MODE == NAIVE_FILE_IO
      {
        ...
        assert(in.is_open());
        while (max_entries == 0 ||
               (max_entries > 0 && entries.size() < max_entries)) {
          in >> entry.doc >> entry.word;
          --entry.doc;
          --entry.word;  // 1-based to 0-based indexing
          if (!in.eof()) {
            in >> entry.count;
    ...
    void fill_doc_word_entries(offset_t max_entries, char* buf,
                               uint64_t fileSize) {
      ...
      int state =
          1;  // Are we trying to read the 1st,2nd or 3rd entry of the line
  ...
  class MMappedOutput {
   ...
   public:
    MMappedOutput(const std::string& filename) {
      ...
      hFile = CreateFileA(filename.c_str(), GENERIC_WRITE | GENERIC_READ, 0,
      ...
      if (fd <= 0) {
        std::cerr << "File " << filename << " coult not be opened or accessed"
                  << std::endl;
      }
#endif
    }
    ...
    ~MMappedOutput() {
      ...
      if (mmap_ptr != 0) {
        std::cerr << "Error: mmap_ptr not null; you forgot to flush_and_close "
                     "this mmap'd file ouptput"
                  << std::endl;
        exit(-1);
      }
      if (fd != 0) {
        std::cerr << "Error: fd not 0; you forgot to flush_and_close this "
                     "mmap'd file ouptput"
                  << std::endl;
        exit(-1);
      }
    ...
    void flush_and_close() {
      ...
      hMapFile = CreateFileMapping(hFile, NULL, PAGE_READWRITE, mmap_size >> 32,
                                   mmap_size % (1ULL << 32), NULL);
      assert(hMapFile != NULL);

      mmap_ptr =
          (char*) MapViewOfFile(hMapFile, FILE_MAP_ALL_ACCESS, 0, 0, mmap_size);
      ...
      mmap_ptr = (char*) mmap(NULL, mmap_size, PROT_READ | PROT_WRITE,
                              MAP_SHARED, fd, 0);
      if (mmap_ptr == MAP_FAILED)
        std::cerr << "mmap for output failed with error: " << errno
                  << std::endl;
#endif
    ...
    inline void reverse(char* const str, const int len) {
      int b = 0;
      int e = len - 1;
      while (b < e) {
    ...
    // Base 10.
    inline char* itoa_mv(int num, char* str, const char terminal) {
      // std::cout << num << "\n";
      int i = 0;
      if (num == 0) {
        str[i++] = '0';
    ...
    inline char* ftoa_mv(FPTYPE num, char* str, const char terminal,
                         const int before_dec = 6, const int after_dec = 6) {
      int i = 0;
      if (num == 0.0) {
        str[i++] = '0';
        str[i++] = '.';
    ...
    template<class FPTYPE>
    void concat_float(FPTYPE num, const char terminal = '\0',
                      const int before_dec = 3, const int after_dec = 6) {
      if (buf_ptr - buf_base > write_buf_size - 100)
        double_buffer();
  ...
  struct quintuple {
    ...
    quintuple(const E& first_, const E& second_, const E& third_,
              const E& fourth_, const E& fifth_)
        : first(first_), second(second_), third(third_), fourth(fourth_),
          fifth(fifth_) {
    }

    inline bool operator==(const quintuple<E>& from) {
      return first == from.first && second == from.second &&
             third == from.third && fourth == from.fourth &&
             fifth == from.fifth;
    }
  };
  ...
  struct quintuple_comp {
    inline bool operator()(const quintuple<T>& l, const quintuple<T>& r) const {
  ...
  void create_vocab_list(const std::string&        vocab_file,
                         std::vector<std::string>& words,
                         const word_id_t           max_words);

  std::string log_dir_name(const doc_id_t     num_topics,
                           const std::string& output_path_base,
                           const bool& sample_docs, const FPTYPE& sample_rate);

  void create_dir(const std::string& dir_name);

  template<class T>
  inline T divide_round_up(const T& num, const T& denom) {
    return (num % denom == 0) ? num / denom : (num / denom) + 1;
  }

  std::string concat_file_path(const std::string& dir,
