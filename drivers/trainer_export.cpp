...
//
// Question: Are doc and word IDs 0-based or 1-based? ISLE assumes 0-base for
// everything.
//
...
//  3. GetNumEdgeTopics(), pre-allocate sufficient space and get EdgeModel to
//  get Edge model
...
namespace ISLE {
  EXPORT_API(ISLETrainer*)
  CreateTrainer(const vocabSz_t vocab_size_, const docsSz_t num_docs_,
                const offset_t max_entries_, const docsSz_t num_topics_,
    ...
    return new ISLETrainer(vocab_size_, num_docs_, max_entries_, num_topics_,
                           sample_docs_, sample_rate_,
                           ISLETrainer::data_ingest::ITERATIVE_DATA_LOAD);
  }
  ...
  EXPORT_API(void)
  feedData(ISLETrainer* trainer, const docsSz_t doc,
           const vocabSz_t* const words, const count_t* const counts,
  ...
  // basicModel must be pre-allocated to size trainer->num_topics *
  // trainer->vocab_size
  //		It consists of 'num_topics' probability distributions over the
  // vocabulary
  //		basicModel[topic*vocab_size + word] provides the weight of 'word' in
  //'topic'.
  //
  ...
  // edgeModel must be pre-allocated to size GetNumEdgeTopics() *
  // trainer->vocab_size
  //		It consists of 'GetNumEdgeTopics()' probability distributions over the
  // vocabulary
  //		edgeModel[topic*vocab_size + word] provides the weight of 'word' in
  //'topic'.
  //
  ...
  EXPORT_API(void)
  GetTopicsCatchWords(ISLETrainer* trainer, const int topic,
                      vocabSz_t* const catchwods, int& numCatchwodsForTopic) {
  }
}
